...
from ..loading import AsyncLoading
...
class DashboardTab(ctk.CTkFrame):
    ...
    def _setup_ui(self):
        ...
        self.btn_transacao = ctk.CTkButton(frame_top, text="1. Carregar Transação", command=self.carregar_transacao)
        self.btn_transacao.pack(side="left", padx=5)

        self.btn_hhs = ctk.CTkButton(frame_top, text="2. Adicionar HHs", fg_color="green", command=self.carregar_hhs, state="disabled")
        self.btn_hhs.pack(side="left", padx=5)
        
        self.btn_consolidar = ctk.CTkButton(frame_top, text="3. Consolidar", fg_color="orange", command=self.consolidar, state="disabled")
        self.btn_consolidar.pack(side="left", padx=5)

        self.btn_cancelar = ctk.CTkButton(frame_top, text="Reiniciar", fg_color="#c0392b", width=80, command=self.reiniciar_fluxo, state="disabled")
        ...
        self.tabela_preview = ctk.CTkScrollableFrame(self, label_text="Prévia da Consolidação")
    ...
    def atualizar_view(self):
        def tarefa():
            return self.service.gerar_relatorio_roi_itm()
        
        def fim(resultado, erro):
            if erro: return
            self._renderizar_cards(resultado)

        AsyncLoading(self.app, tarefa, fim)
    ...
    def carregar_hhs(self):
        ...
        def tarefa():
            return self.service.processar_hhs(caminhos)
    ...
    def _rodar_preview(self):
        def tarefa():
            return self.service.consolidar_dados(self.transacoes_carregadas, self.hhs_carregados)
        
        def fim(resultado, erro):
            if erro: return
            preview, _ = resultado
            self._renderizar_preview(preview)
    ...
    def _renderizar_preview(self, lista):
        ...
        for idx, item in enumerate(vinculados[:30]):
            ...
            ctk.CTkLabel(self.tabela_preview, text="OK", text_color="#3498db").grid(row=idx+1, column=2, padx=5, sticky="w")
